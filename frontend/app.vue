<template>
  <nav class="navbar navbar-expand-lg bg-light">
    <div class="container-fluid">
      ...
      <div class="d-flex">
        ...
        />
        <button class="btn btn-outline-primary" @click="click">Submit</button>
      </div>
    </div>
  </nav>
  ...
  <footer class="py-3 mt-auto">
    <div
      class="container d-flex flex-wrap justify-content-between align-items-center border-top pt-1"
    >
...
const getCourses = async () => {
  console.log("Making call to ", `${config.proxyBaseUrl}/course`);
  const {
    data: { _value },
    error,
  } = await useFetch(`${config.proxyBaseUrl}/course`);
  if (error.value != null && error.value != undefined) {
    console.log("There was an error fetching the courses");
  } else {
    courses.value = _value;
  }
};

const getCourseWithAssignments = async () => {
  const {
    data: { _value },
    pending,
  } = await useLazyFetch(
    `${config.proxyBaseUrl}/course/${selectedCourseId.value}?studentNumber=${studentNumber.value}`
  );
  console.log("Value", _value);
  selectedCourse.value = _value;
};
...
const click = async () => {
  ...
  loading.value = true;
  await getCourseWithAssignments();
  loading.value = false;
};
